...
# n_train = 5000000  # number of training points
# n_features = 100
...
print(prof.display())
print("16-bit precision", np.mean(temp_result), get_max_active_cuda(prof), 'mb')
...
with LineProfiler(topk_low_prec) as prof:
    for i in range(n_iter):
        ...
        temp_result.append(end - start)
print(prof.display())
print("32-bit precision", np.mean(temp_result), get_max_active_cuda(prof), 'mb')
...
with LineProfiler(topk) as prof:
    for i in range(n_iter):
        ...
        temp_result.append(end - start)
print(prof.display())
print("64-bit precision", np.mean(temp_result), get_max_active_cuda(prof), 'mb')
...
#     for i in range(n_iter):
#         start = time.time()
#         clear_pairs_low_prec = neighbor_within_range(X_train_norm, range_threshold=12)
#         end = time.time()
#         print(end - start)
#         temp_result.append(end - start)
# print(prof.display())
...
labels_path = os.path.join("datasets", "fashion-mnist", "train-labels-idx1-ubyte.gz")
images_path = os.path.join("datasets", "fashion-mnist", "train-images-idx3-ubyte.gz")
...
X_train = torch.from_numpy(images).double()
X_train_norm = Standardizer(X_train, return_mean_std=False)
